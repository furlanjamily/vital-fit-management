...
import { GlobalFilters } from "@/components/common/table/global-filters";
import type { TableFilterDefinition } from "@/components/common/table/global-filters.types";
import { TableColGroup } from "@/components/common/table/table-colgroup";
...
type TableProps<T> = {
  ...
  /** @deprecated Use filters com type "text". Mantido para compatibilidade. */
  globalFilter?: string;
  /** @deprecated Use filters. Mantido para compatibilidade. */
  ...
  className?: string;
  /** Itens por página iniciais. Padrão: 10 */
  defaultPageSize?: number;
  /** Opções exibidas no seletor de itens por página. Padrão: [5, 10, 20, 50] */
    ...
    activeFilters.every((filter) => {
      ...
      if (filter.type === "text") {
        ...
        return searchableColumns.some((column) =>
          column.searchValue!(row).toLowerCase().includes(rawValue.toLowerCase()),
        );
      }
...
export function Table<T>({
  ...
  const isLegacyControlled = globalFilter !== undefined;
  const isControlled = filterValues !== undefined || isLegacyControlled;

  const activeFilterValues = useMemo(() => {
    if (isLegacyControlled) {
      return { ...internalFilterValues, search: globalFilter ?? "" };
    }

    return filterValues ?? internalFilterValues;
  }, [filterValues, globalFilter, internalFilterValues, isLegacyControlled]);
  ...
  function handleFilterValueChange(key: string, value: string) {
    const nextValues = { ...activeFilterValues, [key]: value };

    if (onFilterChange) {
      onFilterChange(nextValues);
    } else if (!isLegacyControlled) {
  ...
  function handleClearFilters() {
    const clearedValues = createInitialFilterValues(resolvedFilters);
  ...
  const showGlobalFilters = !isLegacyControlled || filters !== undefined;
  ...
  return (
    ...
    >
      ...
      >
        {(title || (!showGlobalFilters && headerActions)) ? (
          <div className="mb-5 flex shrink-0 items-center justify-between gap-4">
            {title ? <p className="text-sm font-semibold text-white">{title}</p> : <span />}
            {!showGlobalFilters && headerActions ? headerActions : null}
          </div>
        ) : null}
